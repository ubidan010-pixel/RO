...
hlPlayer::hlPlayer(const VideoPlayerInit & playerInit, cell::Sail::memallocator* memoryReplacement, cell::Sail::CustomSource * fileReplacement)
:   mCurrentPlaybackTime(0),
    mAccess(0),
    mEnabled(0)
{
    memallocator *pAllocator = (memoryReplacement) ?
                               memoryReplacement :
                               &sDefaultAllocator;
    CustomSource* pFs = fileReplacement;
	...
	initParams = playerInit;
	pPlayerCore->startup(initParams.pSharedSpurs);
...
bool hlPlayer::Play(const char* videouri, bool looping)
{
    int ret;
	bool bRet;
...
void hlPlayer::updateVideoSync(uint32_t _tag, uint64_t _flipTime)
{
    pPlayerCore->updateVideoSync(_tag, _flipTime);
}
                              ...
                              hlMemAllocator*  allocator)
{
	...
	cell::Sail::hlPlayer* TestPlayerC =
        new (p) cell::Sail::hlPlayer(*initparams, pCustom?pCustom->MemoryReplacement:NULL, pCustom?pCustom->FileReplacement:NULL);
