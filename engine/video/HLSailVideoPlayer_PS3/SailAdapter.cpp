...
// Be sure to get the code runned once with this defined if you implement a custom source
//#define SAIL_CHECK_SOURCE_VALIDITY
 ...
 */
sailAdapter::sailAdapter(CustomSource*  FileReplacement,
                         memallocator* MemoryReplacement) :
    mpChangeRead(0),
    // CHANGE START - fixed audio thread starting too late and reading uninit value
    mPlayerState(CELL_SAIL_PLAYER_STATE_LOST),
    play_state(MP_DONE),
    // CHANGE STOP
    mpFileSrc(FileReplacement),
    ...
    mpActiveSpurs(0)
{
    // lwmutex for pause
    {
        ...
        int ret = sys_lwmutex_create(&mMutex, &attr);
...
{
    int ret;
    ret = ITF::MODULE_MANAGER->loadModule(CELL_SYSMODULE_PAMF);
    if (ret < 0) {
        EMSG("cellSysmoduleLoadModule(CELL_SYSMODULE_PAMF)", ret);
        return false;
    }
    
    ret = ITF::MODULE_MANAGER->loadModule(CELL_SYSMODULE_SAIL);
    if (ret < 0) {
                             ...
                             int height)
{
    ...
    // player
    {
        ...
        //PlayerAttr.playerPreset      = CELL_SAIL_PLAYER_PRESET_AS_IS;
    ...
    if (mpFileSrc)
    {
        mpFileSrc->registerProtocolToPlayer(mPlayer);
    }
...
bool sailAdapter::shutdown(void)
{
    ...
    ret = ITF::MODULE_MANAGER->unloadModule(CELL_SYSMODULE_PAMF);
    DASSERT (ret>=0);
    if (ret < 0) {
    ...
    ret = ITF::MODULE_MANAGER->unloadModule(CELL_SYSMODULE_SAIL);
    DASSERT (ret>=0);
    if (ret < 0) {
...
bool sailAdapter::openStream(char const* pPath)
{
    ...
    if (mpFileSrc)
    {
        strcpy(mUri, mpFileSrc->getProtocolName());
        strcat(mUri, "://");
    }
    else
    {
...
#ifdef SAIL_CHECK_SOURCE_VALIDITY
    if (mpFileSrc)
    {
        ITF_ASSERT(mpFileSrc->check(mUri));
    }
#endif
...
bool sailAdapter::PopulateStreamInfo(void)
{
    ...
    case CELL_SAIL_STREAM_PAMF: {
        ...
        mBeginOffset = (uint64_t(startTime.upper) << 32ull) | uint64_t(startTime.lower);
        ...
        mEndOffset = (uint64_t(endTime.upper) << 32ull) | uint64_t(endTime.lower);
        mMovieDurationMsec = (mEndOffset - mBeginOffset) / 90ull/*kHz*/;
        break;
    }
...
int sailAdapter::onVideoFrameAlloc(size_t size,int num,uint8_t **ppFrame)
{
    ...
    for (int i = 0; i < num; ++i)
    {
        ppFrame[i] = (uint8_t *)mpMemoryAllocator->AllocateTexture(size , 128);
    }
...
int sailAdapter::onVideoFrameFree(int num, uint8_t **ppFrame)
{
    for (int i = 0; i < num; ++i)
    {
        mpMemoryAllocator->DeallocateTexture(ppFrame[i]);
    }
    return CELL_OK;
}
