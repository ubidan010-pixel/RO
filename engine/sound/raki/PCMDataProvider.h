...
#define _ITF_PCMDATAPROVIDER_H_
...
class PCMDataProvider 
    : public raki::SoundStreamDataProvider
{
public:

    PCMDataProvider( const u32 _nbChannels );

    virtual ~PCMDataProvider();

    void reinit();

    void init( MusicInstance * _musicInstance, const MusicPart * _part, raki::PCMStreamSoundNative * _sound );

    virtual void update( bool _launchNewReadRequests );
...
const MusicPart * PCMDataProvider::getCurrentPart()
{
    return m_currentPart;
}


const bbool PCMDataProvider::hasValidStream()
{
