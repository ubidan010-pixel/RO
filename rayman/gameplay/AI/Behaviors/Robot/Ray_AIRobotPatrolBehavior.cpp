...
namespace ITF
{
    IMPLEMENT_OBJECT_RTTI(Ray_AIRobotPatrolBehavior_Template)

    BEGIN_SERIALIZATION_CHILD(Ray_AIRobotPatrolBehavior_Template)
        // ACTIONS
    ...
    END_SERIALIZATION()
        ...
        Ray_AIRobotPatrolBehavior_Template::Ray_AIRobotPatrolBehavior_Template()
        : m_fall(NULL)
        , m_defaultPause(NULL)
        , m_attack(NULL)
        , m_uturnDelay(0.3f)
    ...
    IMPLEMENT_OBJECT_RTTI(Ray_AIRobotPatrolBehavior)
    BEGIN_VALIDATE_BEHAVIOR(Ray_AIRobotPatrolBehavior)
        //VALIDATE_BEHAVIOR_PARAM("fall", m_fall, "this action is mandatory");
    END_VALIDATE_BEHAVIOR()

    Ray_AIRobotPatrolBehavior::Ray_AIRobotPatrolBehavior()
        : Super()
        , m_fall(NULL)
        , m_defaultPause(NULL)
        , m_attack(NULL)
    ...
    void Ray_AIRobotPatrolBehavior::onActorLoaded(Pickable::HotReloadType _hotReload)
    {
    ...
    void Ray_AIRobotPatrolBehavior::onActivate()
    {
        Super::onActivate();
        ...
        startIdle();
        m_physComponent->setStickOnWalls(btrue,bfalse);
    }

    void Ray_AIRobotPatrolBehavior::update( f32 _delta  )
    {
        ...
        //{
        //    // counter gravity
        //    m_physComponent->addForce( -m_aiComponent->getCurrentGravity() );

        //    // add a force normal to the edge
        //    Vec2d groundNormal = m_physComponent->getCurrentGroundDir().getPerpendicular();
        //    m_physComponent->addForce( -groundNormal * 100.0f );


        //}
    }

    void Ray_AIRobotPatrolBehavior::onActionFinished()
    {
    ...
    void Ray_AIRobotPatrolBehavior::onEvent( Event* _event )
    {
    ...
    void Ray_AIRobotPatrolBehavior::updateIdle( f32 _dt )
    {
    ...
    bbool Ray_AIRobotPatrolBehavior::updateMove( f32 _dt )
    {
        PolyLine* poly = m_physComponent->getStickedPolyline();
        ...
        m_physComponent->setStickOnWallsInputDir(m_physComponent->getCurrentGroundDir() * (m_actor->isFlipped()?-1.0f:1.0f));
    ...
    void Ray_AIRobotPatrolBehavior::startAnimPause( )
    {
        if (m_defaultPause)
    ...
    void Ray_AIRobotPatrolBehavior::onActionFinishedFall()
    {
        startIdle();
    }

    void Ray_AIRobotPatrolBehavior::onActionFinishedPause()
    {
        m_timer = getTemplate()->getUturnDelay();
        performUTurnImmediate();
    }
    ...
    void Ray_AIRobotPatrolBehavior::onActionFinishedUturn()
    {
    ...
    void Ray_AIRobotPatrolBehavior::onActionFinishedMove()
    {
        startAnimPause();
    }

    void Ray_AIRobotPatrolBehavior::startIdle()
    {
        setAction(m_idle);
    }

    void Ray_AIRobotPatrolBehavior::startMove( bbool _walkLeft)
    {
        m_move->setWalkLeft(_walkLeft);
        setAction(m_move, btrue);
    }

     void Ray_AIRobotPatrolBehavior::startFall()
    {
        setAction(m_fall);
    }

 
    void Ray_AIRobotPatrolBehavior::updateAnimInputs()
    {
        Super::updateAnimInputs();

    }

    bbool Ray_AIRobotPatrolBehavior::checkBlockingContacts()
    {
    ...
    void Ray_AIRobotPatrolBehavior::processEventBlockedByPolyline( EventBlockedByPolyline* _blockedEvent )
    {
    ...
    bbool Ray_AIRobotPatrolBehavior::checkHole( PolyLine* poly )
    {
        f32 holeDepth;
    ...
    ITF::bbool Ray_AIRobotPatrolBehavior::checkUturn()
    {
    ...
    ITF::bbool Ray_AIRobotPatrolBehavior::checkPolylineObstacles( PolyLine* poly, u32 edgeIndex )
    {
        ...
        {
            const PolyLineEdge& edge = poly->getEdgeAt(nextEdgeIndex);
            Vec2d edgeDir = edge.m_normalizedVector.Rotate(-m_physComponent->getCurrentGroundDir().getAngle());
            edgeType = AIUtils::getEdgeType(edgeDir);
            up = m_physComponent->getCurrentGroundDir().getPerpendicular().dot(edge.m_normalizedVector) > 0.0f;
            up = m_actor->isFlipped()?!up:up;
    ...
    ITF::bbool Ray_AIRobotPatrolBehavior::checkBlockingContact(ObjectRef _poly, u32 _edgeIndex, Vec2d _contact)
    {
    ...
    void Ray_AIRobotPatrolBehavior::onActionFinishedBounceToLayer()
    {
        Super::onActionFinishedBounceToLayer(  );
        startIdle();
    }

    void Ray_AIRobotPatrolBehavior::onActionFinishedBounce()
    {
        Super::onActionFinishedBounce(  );
        startIdle();
    }

    bbool Ray_AIRobotPatrolBehavior::checkAttack()
    {
        if (Actor * enemy = AIUtils::getClosestEnemy(m_actor,m_actor->isFlipped(),getTemplate()->getDetectionRange()))
        {
            startAttack();
    ...
    void Ray_AIRobotPatrolBehavior::startAttack()
    {
        setAction(m_attack);
    }

    void Ray_AIRobotPatrolBehavior::onActionFinishedAttack()
    {
        startIdle();
    }

    void Ray_AIRobotPatrolBehavior::onDeactivate()
    {
        Super::onDeactivate(  );
    ...
    void Ray_AIRobotPatrolBehavior::drawEdit( ActorDrawEditInterface* _drawInterface, u32 _flags ) const
    {
