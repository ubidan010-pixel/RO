...
namespace ITF
{
    IMPLEMENT_OBJECT_RTTI(Ray_AIAlInfernoPatrolBehavior_Template)

    BEGIN_SERIALIZATION_CHILD(Ray_AIAlInfernoPatrolBehavior_Template)
        // ACTIONS
    ...
    END_SERIALIZATION()
        ...
        Ray_AIAlInfernoPatrolBehavior_Template::Ray_AIAlInfernoPatrolBehavior_Template()
        : m_fall(NULL)
        , m_defaultPause(NULL)
        , m_attack(NULL)
        , m_uturnDelay(0.3f)
    ...
    IMPLEMENT_OBJECT_RTTI(Ray_AIAlInfernoPatrolBehavior)
    BEGIN_VALIDATE_BEHAVIOR(Ray_AIAlInfernoPatrolBehavior)
        //VALIDATE_BEHAVIOR_PARAM("fall", m_fall, "this action is mandatory");
    END_VALIDATE_BEHAVIOR()

    Ray_AIAlInfernoPatrolBehavior::Ray_AIAlInfernoPatrolBehavior()
        : Super()
        , m_fall(NULL)
        , m_defaultPause(NULL)
        , m_attack(NULL)
    ...
    void Ray_AIAlInfernoPatrolBehavior::onActorLoaded(Pickable::HotReloadType _hotReload)
    {
    ...
    void Ray_AIAlInfernoPatrolBehavior::onActivate()
    {
        Super::onActivate();
    ...
    void Ray_AIAlInfernoPatrolBehavior::update( f32 _delta  )
    {
    ...
    void Ray_AIAlInfernoPatrolBehavior::onActionFinished()
    {
    ...
    void Ray_AIAlInfernoPatrolBehavior::onEvent( Event* _event )
    {
    ...
    void Ray_AIAlInfernoPatrolBehavior::updateIdle( f32 _dt )
    {
    ...
    bbool Ray_AIAlInfernoPatrolBehavior::updateMove( f32 _dt )
    {
        PolyLine* poly = m_physComponent->getStickedPolyline();
    ...
    void Ray_AIAlInfernoPatrolBehavior::startAnimPause( )
    {
        if (m_defaultPause)
    ...
    void Ray_AIAlInfernoPatrolBehavior::onActionFinishedFall()
    {
        startIdle();
    }

    void Ray_AIAlInfernoPatrolBehavior::onActionFinishedPause()
    {
        m_timer = getTemplate()->getUturnDelay();
        performUTurnImmediate();
    }
    ...
    void Ray_AIAlInfernoPatrolBehavior::onActionFinishedUturn()
    {
    ...
    void Ray_AIAlInfernoPatrolBehavior::onActionFinishedMove()
    {
        startAnimPause();
    }

    void Ray_AIAlInfernoPatrolBehavior::startIdle()
    {
        setAction(m_idle);
    }

    void Ray_AIAlInfernoPatrolBehavior::startMove( bbool _walkLeft)
    {
        m_move->setWalkLeft(_walkLeft);
        setAction(m_move, btrue);
    }

     void Ray_AIAlInfernoPatrolBehavior::startFall()
    {
        setAction(m_fall);
    }

 
    void Ray_AIAlInfernoPatrolBehavior::updateAnimInputs()
    {
        Super::updateAnimInputs();

    }

    bbool Ray_AIAlInfernoPatrolBehavior::checkBlockingContacts()
    {
    ...
    void Ray_AIAlInfernoPatrolBehavior::processEventBlockedByPolyline( EventBlockedByPolyline* _blockedEvent )
    {
    ...
    bbool Ray_AIAlInfernoPatrolBehavior::checkHole( PolyLine* poly )
    {
        f32 holeDepth;
    ...
    ITF::bbool Ray_AIAlInfernoPatrolBehavior::checkUturn()
    {
    ...
    ITF::bbool Ray_AIAlInfernoPatrolBehavior::checkPolylineObstacles( PolyLine* poly, u32 edgeIndex )
    {
    ...
    ITF::bbool Ray_AIAlInfernoPatrolBehavior::checkBlockingContact(ObjectRef _poly, u32 _edgeIndex, Vec2d _contact)
    {
    ...
    void Ray_AIAlInfernoPatrolBehavior::onActionFinishedBounceToLayer()
    {
        Super::onActionFinishedBounceToLayer(  );
        startIdle();
    }

    void Ray_AIAlInfernoPatrolBehavior::onActionFinishedBounce()
    {
        Super::onActionFinishedBounce(  );
        startIdle();
    }

    bbool Ray_AIAlInfernoPatrolBehavior::checkAttack()
    {
        if (Actor * enemy = AIUtils::getClosestEnemy(m_actor,m_actor->isFlipped(),getTemplate()->getDetectionRange()))
        {
            startAttack();
    ...
    void Ray_AIAlInfernoPatrolBehavior::startAttack()
    {
        setAction(m_attack);
    }

    void Ray_AIAlInfernoPatrolBehavior::onActionFinishedAttack()
    {
        startIdle();
    }
    ...
    void Ray_AIAlInfernoPatrolBehavior::drawEdit( ActorDrawEditInterface* _drawInterface, u32 _flags ) const
    {
