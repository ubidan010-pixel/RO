...
#include "MemMngBucketsPrivate_PS3.h"
...
// Check validity
//////////////////////////////////////////////////////////////////////////
void	GlobalCheckValidity(BucketHeader & headerBucket)
{
#if CHECK_BUCKETS_POLICY!=0
	static int TEST_EVERY= 10000;
	static int nCount= 0;
	nCount++;
	if (nCount<TEST_EVERY)
	{
		return;
	}
	nCount= 0;
...
void* Alloc(u32 iSize, u32 uAlign)
{
	...
	if (pheaderBucket!=NULL)
	{
		GlobalCheckValidity(*pheaderBucket);
		void* pRet= pheaderBucket->AllocElt();
	...
	return NULL; // no out of mem report here (caller's job)
}
...
void Free(void *pData)
{
	...
	GlobalCheckValidity(*headerBucket);
...
u32 MSize(void * pData)
{
    ...
    if (headerBucket)
    {
	    ...
	    u32 nSize= headerBucket->GetEltSize();
	    GlobalCheckValidity(*headerBucket);
	    return nSize;
    }
