...
// PS3-specific chunk allocator for big and temporary allocations (based on the VRAMAllocator from PS3).
// beware of reentrance -> hash_map elements are allocated, so they should not be allocated there
...
namespace ITF
{
    ...
    namespace ChunkAllocator
    {
        ...
        public:
            ...
            bool IsAddressInRange(MemAddress addr) const { return ((u32)(addr - (MemAddress)m_startAddr) < m_initialSize); }
            ...
            u32 GetTotalSize()  { return m_stat_totalUsedSize; }
            u32 GetTotalPeakSize()  { return m_stat_totalPeakSize; }
        ...
        private:
            ...
            std::hash_map<MemAddress, ChunkIdx> m_addr2AllocatedChunk;  // map of addresses to the chunk
        ...
        {
            std::hash_map<MemAddress, ChunkIdx>::iterator it = m_addr2AllocatedChunk.find(addr);
        ...
        ChunkIdx ChunkContainer::CreateChunk()
        {
            ...
            if (!IsValid(m_firstNotUsedEntry))
            {
                ...
                ChunkEntry * first = &(*m_chunks.begin());
    ...
    bool Init()
    {
        ...
        bigMemoryPageManager.Init(256u*1024*1024, MemoryPageManager::SMALL_PAGE_SIZE);
    ...
    ux              GetPageSize()
    {
        return MemoryPageManager::SMALL_PAGE_SIZE;
    }
    ...
    void*			Alloc(u32 _size)
    {
        ...
        u8 * committed = g_bigMemoryPageManager->CommitArea(requestedAddress, _size);
        ...
        {
            FILL_ALLOC(committed, g_chunkContainer->GetChunkSize(chkIdx));
        }
    ...
    void			Free(void * _ptr)
    {
        ...
        FILL_FREE(_ptr, g_chunkContainer->GetChunkSize(chkIdx));
        g_chunkContainer->Free(chkIdx);
        g_bigMemoryPageManager->DecommitArea((u8 *)_ptr);
    }
