...
public:
		...
		)
	{
		...
		if (in_eOpenMode == AK_OpenModeRead)
		{
			...
			(void)nn::fs::GetFileSize(&out_fileDesc.iFileSize, out_fileDesc.hFile);
		}
	...
	{
		// NX API asserts on paths containing backslashes. Convert backslashes to slashes automatically.
		AkOSChar* c = io_path;
		while (*c != '\0')
		{
			if (*c == '\\')
