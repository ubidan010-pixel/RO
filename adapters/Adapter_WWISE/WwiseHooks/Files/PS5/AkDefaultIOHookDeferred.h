...
// Uses APR to queue reads asynchronously, and creates another thread to handle
// callbacks on IO completion.
//
...
public:
	...
	struct AkPS5FileDesc : public AkFileDesc
	{
		AkPS5FileDesc() {};
		AkPS5FileDesc(const AkPS5FileDesc& in_rCopy)
			: AkFileDesc(in_rCopy)
			, idApr(in_rCopy.idApr)
		{}
		SceAprFileId idApr = 0;
	};
	typedef AkPS5FileDesc AkFileDescType;
...
protected:
	...
	bool BatchOpenWorker(AkOSChar* in_szBasePath, AkUInt32 in_uNumFiles, AkAsyncFileOpenData** in_ppItems, AKRESULT* io_pDispatchResults);

	AkDeviceID			m_deviceID;
		
	struct AprBatch
	{
		SceAprSubmitId submitId; // ampr submitId in case we need to wait on it
		SceAprResultBuffer resultBuffer; // storage of the apr read results

		AkUInt32 uNumTransferInfos; // size of ppTransferInfos array
		AkAsyncIOTransferInfo** ppTransferInfos; // ppTransferInfos array to pass to batchIoCallback
		volatile bool bAvailableForWrite;
	};
	AprBatch*			m_pAprBatches;
	AkUInt32			m_uNextAprBatchIdx;
	void* m_pAprCmdBufferData;	// storage of all aprBatch command buffers
	AkAsyncIOTransferInfo** m_ppAprTransferInfoData; // storage of the AprBatch::ppTransferInfo data

	void SubmitAprReads(
		AkUInt32 numTransferInfos,
		AkAsyncIOTransferInfo** ppTransferInfos,
		SceAprFileId* pAprFileIds);
	void CompleteAprBatch(AprBatch* pAprBatch);

	static AK_DECLARE_THREAD_ROUTINE(AkDefaultIOHookDeferredCallback);
	AkThread			m_callbackThread;
	SceKernelEqueue		m_callbackEventQueue;
