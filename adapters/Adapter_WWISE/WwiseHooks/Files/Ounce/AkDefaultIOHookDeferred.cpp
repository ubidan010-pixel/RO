...
// Device info.
#define OUNCE_DEFERRED_DEVICE_NAME		AKTEXT("Ounce Deferred")	// Default deferred device name.
...
{
	AKASSERT(io_transferInfo.pBuffer && io_transferInfo.uRequestedSize > 0);

	AkUInt32 iReadSize = 0;
	CAkFileHelpers::ReadBlocking(in_fileDesc.hFile, io_transferInfo.pBuffer, (AkUInt32)io_transferInfo.uFilePosition, io_transferInfo.uRequestedSize, iReadSize);

	AKRESULT eResult = iReadSize ? AK_Success : AK_Fail;
...
{
	AKASSERT(io_transferInfo.pBuffer && io_transferInfo.uRequestedSize > 0);
	AKRESULT eResult = CAkFileHelpers::WriteBlocking(in_fileDesc.hFile, io_transferInfo.pBuffer, (AkUInt32)io_transferInfo.uFilePosition, io_transferInfo.uRequestedSize);
    ...
    )
{
	if (in_pFileDesc)
	{
		CAkFileHelpers::CloseFile(in_pFileDesc->hFile);
    ...
    )
{
	...
	AK_OSCHAR_TO_UTF16(out_deviceDesc.szDeviceName, OUNCE_DEFERRED_DEVICE_NAME, AK_MONITOR_DEVICENAME_MAXLENGTH);
