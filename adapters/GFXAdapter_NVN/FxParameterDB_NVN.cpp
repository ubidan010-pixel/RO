...
namespace ITF
{
    ...
    FxParameterDB::TexSamplerHdl FxParameterDB::registerTexSampler(const char * _name, NVNSamplerPool& _samplerPool)
    {
        return TexSamplerHdl{ m_texAndSamplers.add(_name, TextureAndSamplerDesc(_samplerPool)) };
    }
    ...
    void FxParameterDB::SamplerDesc::updateSamplerHandle(NVNSamplerPool& _samplerPool)
    {
        m_samplerHdl = _samplerPool.getSamplerHandle(m_wrapModeU, m_wrapModeV, m_linearFiltering);
    }

    // set the shader parameters in the DB

    void FxParameterDB::setTexture(TexSamplerHdl _hdl, NVNTexture* _tex)
    {
        auto& texAndSampler = m_texAndSamplers.get(_hdl.getIdx());
        texAndSampler.texture = _tex;
        texAndSampler.textureHdl = _tex == nullptr ? nvn::SeparateTextureHandle{} : _tex->getTextureHandle();
    }

    void FxParameterDB::setSampler(TexSamplerHdl _hdl, const SamplerDesc& _samplerDesc)
    {
        m_texAndSamplers.get(_hdl.getIdx()).samplerDesc = _samplerDesc;
    }
    ...
    nvn::SeparateTextureHandle FxParameterDB::getTextureHdl(TexSamplerHdl _hdl) const
    {
        return m_texAndSamplers.get(_hdl.getIdx()).textureHdl;
    }

    NVNTexture * FxParameterDB::getTexture(FxParameterDB::TexSamplerHdl _hdl) const
    {
        return m_texAndSamplers.get(_hdl.getIdx()).texture;
    }

    nvn::SeparateSamplerHandle FxParameterDB::getSamplerHdl(TexSamplerHdl _hdl) const
    {
        return m_texAndSamplers.get(_hdl.getIdx()).samplerDesc.getSamplerHandle();
    }

    FxParameterDB::SamplerDesc FxParameterDB::getSamplerDesc(TexSamplerHdl _hdl) const
    {
        return m_texAndSamplers.get(_hdl.getIdx()).samplerDesc;
    }
