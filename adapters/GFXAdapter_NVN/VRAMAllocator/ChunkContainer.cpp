...
#include "Core/memory/MemMng/MemMngTracer.h"

#include "core/math/MathTools.h"

// MEMORY tracking 
#define MEMMNG_TRACER_ALLOC_VRAM(x, size) MEMMNG_TRACER_ALLOC_SIZE(x, size)
#define MEMMNG_TRACER_FREE_VRAM(x, size) MEMMNG_TRACER_FREE(x, size);
...
namespace ITF::NVN::VRAMAllocator
{
    ...
    void ChunkContainer::Init(nvn::MemoryPoolBuilder poolBuilder, bool forceOOMBreak, bool verboseOutOfMem, bool staticBuffer)
    {
        VRAMAddress startOfBlock = reinterpret_cast<VRAMAddress>(poolBuilder.GetMemory());
        ChunkSize blockSize = ChunkSize(poolBuilder.GetSize());

        ITF_ASSERT(m_memoryPool == nullptr);

        m_memoryPool = makeBuilderInitialized<nvn::MemoryPool>(poolBuilder);
